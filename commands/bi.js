...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('bi')
		.setDescription('Apply a bi flag ring to a user\'s avatar')
	...
	async execute(interaction, client) {
		...
		let av = `https://some-random-api.ml/canvas/bisexual?avatar=${user.avatarURL({ size: 4096, dynamic: true,format: 'png' })}`    
	...
	if (res.statusCode !== 200) { //check if it isnt 200 OK
		...
		let attachment = new MessageAttachment(res.body, `${interaction.user.username}-bi.png`) //make attachment using the buffer
