...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('pan')
		.setDescription('Apply a pansexual flag ring to a user\'s avatar')
	...
	async execute(interaction, client) {
		...
		let av = `https://some-random-api.ml/canvas/pansexual?avatar=${user.avatarURL({ size: 4096, dynamic: true,format: 'png' })}`    
	...
	if (res.statusCode !== 200) { //check if it isnt 200 OK
		...
		let attachment = new MessageAttachment(res.body, `${interaction.user.username}-pan.png`) //make attachment using the buffer
